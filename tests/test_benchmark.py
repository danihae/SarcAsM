...
class TestMotionBenchmark:
    """Benchmark the motion (LOI) pipeline."""

    @pytest.mark.benchmark
    @pytest.mark.slow
    def test_full_motion_pipeline(self, motion_file_path, benchmark_output_dir):
        result = BenchmarkResult.for_test("full_motion_pipeline", motion_file_path)
        ...
        with timer(result, "full_pipeline"):
            ...
            result.add_metadata("loi_analyzed", bool(lois))
            if lois:
                file, roi = lois[0]
                Motion(file, roi).full_analysis_loi()
            else:
                logger.warning("No LOIs detected, skipping motion analysis")

        filepath = result.save_json(benchmark_output_dir, prefix="motion_")
        print(f"[bench] saved {filepath.name}  | total {result.total_time:.3f}s")

    @pytest.mark.benchmark
    @pytest.mark.slow
    def test_motion_substeps(self, motion_file_path, benchmark_output_dir):
        ...
        with timer(result, "detect_sarcomeres"):
            sarc.detect_sarcomeres(frames=0, max_patch_size=(256, 1024))
        with timer(result, "detect_z_bands_fast_movie"):
            sarc.detect_z_bands_fast_movie(max_patch_size=(32, 210, 1024))
        with timer(result, "analyze_sarcomere_vectors"):
            sarc.analyze_sarcomere_vectors(frames=0)
        with timer(result, "detect_lois"):
            sarc.delete_lois()
            sarc.detect_lois(n_lois=2, persistence=6, mode="fit_straight_line")

        lois = sarc.get_list_lois()
        result.add_metadata("n_lois_detected", len(lois))
        if lois:
            file, roi = lois[0]
            mot_obj = Motion(file, roi)
            with timer(result, "motion_full_analysis_loi"):
    ...
    @pytest.mark.benchmark
    @pytest.mark.slow
    def test_loi_detection_only(self, motion_file_path, benchmark_output_dir):
        ...
        filepath = result.save_json(benchmark_output_dir, prefix="motion_")
        print(f"[bench] saved {filepath.name}  | LOIs={len(lois)}  | total {result.total_time:.3f}s")
