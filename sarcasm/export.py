...
class MultiStructureAnalysis:
    ...
    def get_data(self, structure_keys=None, meta_keys=None):
        ...
        for i, tif_file in enumerate(tqdm(self.files)):
            try:
                ...
                dict_i = MultiStructureAnalysis.get_structure_dict(sarc_obj, meta_keys, structure_keys,
    ...
    def get_structure_dict(sarc_obj: SarcAsM, meta_keys=None, structure_keys=None, **conditions):
        ...
        if structure_keys is None:
            structure_keys = MultiStructureAnalysis.structure_keys_default
        if meta_keys is None:
            meta_keys = MultiStructureAnalysis.meta_keys_default
        ...
        dict_structure_select = {key: sarc_obj.structure.data[key] if key in sarc_obj.structure.data else np.nan for key in
                                 structure_keys}
...
class MultiLOIAnalysis:
    ...
    def get_data(self, loi_keys=None, meta_keys=None):
        ...
        for tif_file, loi_name in tqdm(self.lois):
            try:
                ...
                dict_i = MultiLOIAnalysis.get_motion_dict(motion_obj, meta_keys, loi_keys, **self.conditions)
                self.data.append(dict_i)
            except Exception as e:
    ...
    def get_motion_dict(motion_obj: Motion, meta_keys=None, loi_keys=None, concat=False, **conditions):
        ...
        # get data from metadata and structure dicts
        if loi_keys is None:
            loi_keys = MultiLOIAnalysis.loi_keys_default
        if meta_keys is None:
            meta_keys = MultiLOIAnalysis.meta_keys_default
